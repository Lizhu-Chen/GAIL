...
class Dset(object):
    def __init__(self, inputs, labels, randomize):
        ...
        self.num_paris = len(inputs)
        self.init_pointer()
        self.pointer = 0

    def init_pointer(self):
        if self.randomize:
            idx = np.arange(self.num_paris)
    ...
    def get_next_batch(self, batch_size):
        ...
        if self.pointer + batch_size >= self.num_paris:
            self.init_pointer()
...
class Mujoco_Dset(object):
    def __init__(self, expert_path, train_fraction=0.7, traj_limitation=10, randomize=True):
    ...
    def log_info(self):
        ...
        logger.log("Total transition: %d" % self.num_transition)
