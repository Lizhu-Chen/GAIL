...
class MlpPolicy(object):  # multi-layer perceptron 政策函数，本质上是个神经网络
    recurrent = False     # 这个函数是用来输入状态，计算出动作的分布以及输出最优动作
    ...
    def _init(self, ob_space, ac_space, hid_size, num_hid_layers, gaussian_fixed_var=True):
        ...
        if gaussian_fixed_var and isinstance(ac_space, gym.spaces.Box):
            ...
            pdparam = tf.concat([mean, mean * 0.0 + logstd], axis=1)  # 动作分布的方差和平均值
        else:
